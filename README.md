...
- 👥 **Команды и роли** - управляйте подписками в команде, назначайте роли (администратор/пользователь)
- 📧 **Приглашения** - приглашайте участников по email с настройкой прав доступа
- 💼 **Бюджетирование** - устанавливайте бюджеты для команд и контролируйте расходы
...
## Применение миграций к существующей базе данных

Если вы обновили проект и нужно применить новые миграции к уже работающей базе данных, используйте один из следующих скриптов:

### Для Windows:

Просто запустите файл `apply-migrations.bat` двойным кликом или из командной строки:
...
1. Копируют файлы миграций в контейнер PostgreSQL
2. Применяют все миграции к базе данных
3. Перезапускают приложение
4. Очищают временные файлы

**Важно:** Миграции безопасно применяются с использованием `IF NOT EXISTS`, поэтому их можно запускать повторно без риска ошибок.
...
### Работа с командами и приглашениями

FlowPay поддерживает командную работу с системой ролей и приглашений:

#### Роли пользователей:

- **Администратор** - полный доступ к управлению подписками, командами и пользователями
- **Пользователь** - доступ только к подпискам своих команд

#### Создание команды:

1. Нажмите на кнопку "👥 Команды" в верхней панели
2. Нажмите "Создать команду"
3. Заполните информацию:
   - Название команды (например, "Маркетинг", "Семья")
   - Описание (опционально)
   - Бюджет (опционально) - установите лимит расходов
4. Нажмите "Сохранить"

#### Приглашение участников:

1. Откройте команду, кликнув на её карточку
2. Нажмите "➕ Пригласить"
3. Заполните форму:
   - Email пользователя
   - Роль (Администратор или Пользователь)
4. Нажмите "Отправить приглашение"
5. Пользователь получит ссылку-приглашение для регистрации

#### Принятие приглашения:

1. Пользователь переходит по ссылке из приглашения
2. Видит детали приглашения (роль, команду, отправителя)
3. Вводит своё имя и пароль
4. Нажимает "Принять приглашение"
5. Автоматически входит в систему с назначенной ролью

#### Управление участниками:

- Просмотр списка участников команды
- Удаление участников из команды (только владелец)
- Отмена отправленных приглашений
...
GET    /api/invitations        - Получить мои приглашения
GET    /api/invitations/:token - Получить приглашение по токену (публичный)
POST   /api/invitations/accept - Принять приглашение (публичный)
DELETE /api/invitations/:token - Удалить/отменить приглашение
```

### Отмена подписок

```
GET /api/cancellation/instructions              - Получить все инструкции
GET /api/cancellation/instructions/:serviceName - Получить инструкцию для сервиса
POST /api/subscriptions/:id/cancel              - Отметить подписку как отмененную
```
